          ...
          <div className="-mx-4 sticky top-0 z-40 border-b border-[rgba(15,77,50,0.08)] bg-[rgba(255,255,255,0.92)]/95 px-4 py-3 backdrop-blur-xl">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--green)]">Mục sản phẩm</div>
                <div className="truncate text-[16px] font-semibold leading-tight text-[var(--green-dark)]">Khám phá theo nhóm</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white px-4 pb-6 pt-2">
          ...
          {filteredProducts.length === 0 ? (
            <div className="mt-6 rounded-[22px] border border-[rgba(15,77,50,0.12)] bg-[rgba(15,77,50,0.04)] p-5 text-sm leading-7 text-[var(--muted)]">
              Không tìm thấy sản phẩm phù hợp. Thử đổi từ khóa hoặc chọn mục khác để xem gợi ý.
            </div>
          ) : null}
        </div>
      </section>
      ...
      <section className="container hidden md:block section pt-10 md:pt-14">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 max-w-[60ch] text-[15px] leading-8 text-[var(--muted)] md:text-base">
            Trang mục sản phẩm được tối ưu cho mobile theo phong cách khám phá nhanh, còn trên desktop vẫn giữ nhịp đọc
            rõ ràng và giàu SEO.
          </p>
        </div>
              ...
              <article key={product.slug} className="card overflow-hidden rounded-[28px]">
                <Link href={`/san-pham/${product.slug}`}>
                  ...
                  <div className="p-5">
                    ...
                    <p className="mt-3 text-sm leading-7 text-[var(--muted)]">{product.shortDescription}</p>
                  </div>
                </Link>
              </article>
            );
