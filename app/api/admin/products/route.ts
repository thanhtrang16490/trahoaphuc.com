...
export async function POST(request: Request) {
  ...
  if (!product.slug || !product.name || !product.category_id || !Number.isInteger(product.stock_quantity) || product.stock_quantity < 0 || !Number.isInteger(product.low_stock_threshold) || product.low_stock_threshold < 0 || !Number.isInteger(price) || price <= 0 || !Number.isInteger(originalPrice) || originalPrice < price) return apiError("Thiếu hoặc sai thông tin sản phẩm. Giá gốc phải lớn hơn hoặc bằng giá bán.", 422);
  ...
  if (warehouse) await context.admin.from("inventory_levels").insert({ warehouse_id: warehouse.id, product_id: data.id, quantity: product.stock_quantity });
