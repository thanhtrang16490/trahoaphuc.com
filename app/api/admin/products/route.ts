...
export async function POST(request: Request) {
  ...
  if (!product.slug || !product.name || !product.category_id || !Number.isInteger(product.stock_quantity) || product.stock_quantity < 0 || !Number.isInteger(price) || price <= 0) return apiError("Thiếu hoặc sai thông tin sản phẩm.", 422);
  ...
  const { error: priceError } = await context.admin.from("product_prices").insert({ product_id: data.id, price_vnd: price, currency: "VND" });
