...
import { AdminCouponCreateForm, AdminNewsCreateForm, AdminProductCreateForm, AdminProductManager, AdminUserManager, AdminOrderManager } from "@/components/admin-crud-panels";

type AdminData = {
  role: string;
  user_id: string;
  categories: Array<{ id: string; name: string }>;
  products: Array<{ id: string; name: string; slug: string; category_id: string; short_description?: string; long_description?: string; package_label?: string; image?: string; box_image?: string; origin?: string; stock_quantity: number; is_active: boolean; product_prices?: Array<{ price_vnd: number }> }>;
  orders: Array<{ id: string; order_number: string; customer_name: string; customer_email: string; customer_phone: string; recipient_name: string; recipient_email: string; recipient_phone: string; shipping_address: string; shipping_note: string; total_vnd: number; subtotal_vnd: number; shipping_fee_vnd: number; discount_vnd: number; status: string; payment_status: string; payment_method: string; coupon_code: string | null; created_at: string; order_items: Array<{ id: string; product_name: string; product_slug: string; unit_price_vnd: number; quantity: number; line_total_vnd: number }> }>;
...
const tabs = ["Tổng quan", "Sản phẩm", "Đơn hàng", "Tài khoản", "Lead", "Coupon", "Tin tức"];
